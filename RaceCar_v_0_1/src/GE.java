 ...
 * 
 * Note:
 * 	This is game engine that extends some of the functions of the EZGraphics library
 *  adding sprite animation and limited 2d physics. There are methods for raycasting
...
class NavNode {
	...
	public NavNode( double x, double y ) {
		...
		this.graphic = EZ.addCircle((int)this.position.x, (int)this.position.y, 20, 20, new Color(0,150,150,255), true);
...
class PhysicsManager {
	...
	private static double angularDragCoeff = 0.5;
	...
	private PhysicsManager(int max_c_o) {
		...
		max_velocity = 10; // in meters per second
	...
	public static void applyVelocity( Collider c ) {
		...
		c.graphic.getParent().translateBy(
				c.velocity.x*meter*EZ.getDeltaTime()/1000,
				c.velocity.y*meter*EZ.getDeltaTime()/1000
		);
		...
		if(!in) {
			...
			if(bb0[0].x <= bb1[1].x && bb0[1].x >= bb1[0].x &&
				bb0[0].y <= bb1[1].y && bb0[1].y >= bb1[0].y ) {
					...
					if(obj1.type != 1)
						obj0.hitObjectList.add(obj1);
					if(obj0.type != 1)
						obj1.hitObjectList.add(obj0);
	...
	public static void applyCollisionVelocity(Collider obj0, Collider obj1, Vector MTV, Vector colPoint, boolean angle_only) {
		...
		// angular velocity
		contactVel = relVelw.dot(MTV.copy());
		...
		if(contactVel < 0) {
			...
			double temp0 = (1/obj0.momentInertia)*r0.compOn(impulse);
			double temp1 = (1/obj1.momentInertia)*r1.cross(impulse);
...
class Collider {
	...
	public double areaInertiaTri(Vector v0, Vector v1, Vector v2) {
		...
		return ((b*b*b*h)-(b*b*h*a)+(b*h*a*a)+(b*h*h*h))/36.0;
	}
	/**
...
class Mouse implements MouseListener, MouseMotionListener{
	...
	public static Vector mouse_pos = new Vector(0,0);
	...
	public void mouseDragged(MouseEvent e) {
		...
		group.translateTo(mouse_pos.x, mouse_pos.y);
